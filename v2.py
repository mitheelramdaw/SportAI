...
# Draw small direction arrow at ankle
...
# Draw feet circle
def draw_feet_circle(frame, landmarks, pid):
    try:
...
def analyse_video(input_path, output_path):
    ...
    with mp_pose.Pose(static_image_mode=False, model_complexity=1) as pose:
        while cap.isOpened():
            ...
            # Invalidate stale heatmaps
            for pid in list(heatmaps.keys()):
                if frame_count - last_movement_time[pid] > int(fps * 3):
                    heatmaps[pid] *= 0.95  # fade out gradually
            ...
            if pose_results.pose_landmarks:
                for i, det in enumerate(results.boxes):
                    ...
                    # Feet and arrow
                    feet_point = draw_feet_circle(frame, pose_results.pose_landmarks.landmark, pid)
                    if feet_point:
                        heatmaps[pid] = cv2.circle(heatmaps[pid], feet_point, 10, 1, -1)
                        last_movement_time[pid] = frame_count

            # Draw combined fading heatmaps
            total_heat = np.sum(np.stack(list(heatmaps.values()), axis=0), axis=0)
...
if uploaded_file:
    ...
    with st.spinner("Analysing match video with Smart AI..."):
        analyse_video(input_path, output_path)

    st.success("✅ Analysis complete – here’s your enhanced video:")
    st.video(output_path)

    with open(output_path, "rb") as f:
        st.download_button("📥 Download Analysed Video", f, file_name="enhanced_analysis.mp4", mime="video/mp4")
